...
def add_company():
    ...
    conn = connect_db()
    cursor = conn.cursor()
    cursor.execute("INSERT INTO company (name, tax_number) VALUES (?, ?)", (company_name, tax_number))
    conn.commit()
    conn.close()
    messagebox.showinfo("Success", "تم إضافة الشركة بنجاح")
...
# واجهة Tkinter
root = tk.Tk()
root.title("إضافة شحنة وشركة")

tk.Label(root, text="اسم الشركة:").grid(row=0, column=0)
company_name_entry = tk.Entry(root)
company_name_entry.grid(row=0, column=1)

tk.Label(root, text="الرقم الضريبي:").grid(row=1, column=0)
tax_number_entry = tk.Entry(root)
tax_number_entry.grid(row=1, column=1)

tk.Label(root, text="اسم المركب:").grid(row=2, column=0)
vessel_name_entry = tk.Entry(root)
vessel_name_entry.grid(row=2, column=1)

tk.Label(root, text="رقم بوليصة الشحن:").grid(row=3, column=0)
bill_of_lading_entry = tk.Entry(root)
bill_of_lading_entry.grid(row=3, column=1)

tk.Label(root, text="تاريخ استلام الأوراق:").grid(row=4, column=0)
documents_received_date_entry = tk.Entry(root)
documents_received_date_entry.grid(row=4, column=1)

tk.Label(root, text="تاريخ الوصول المتوقع:").grid(row=5, column=0)
expected_arrival_date_entry = tk.Entry(root)
expected_arrival_date_entry.grid(row=5, column=1)

tk.Label(root, text="الوزن (كجم):").grid(row=6, column=0)
weight_entry = tk.Entry(root)
weight_entry.grid(row=6, column=1)

tk.Label(root, text="عدد الطرود:").grid(row=7, column=0)
packages_count_entry = tk.Entry(root)
packages_count_entry.grid(row=7, column=1)

tk.Label(root, text="رقم الفاتورة:").grid(row=8, column=0)
invoice_number_entry = tk.Entry(root)
invoice_number_entry.grid(row=8, column=1)

tk.Button(root, text="إضافة الشحنة", command=add_shipment).grid(row=9, column=1)
tk.Button(root, text="إضافة الشركة", command=add_company).grid(row=10, column=1)
